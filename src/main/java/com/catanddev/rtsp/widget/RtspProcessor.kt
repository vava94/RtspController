...
import com.catanddev.rtsp.RtspClient.SdpInfo
...
import com.catanddev.rtsp.codec.AudioCodecType
import com.catanddev.rtsp.codec.AudioDecodeThread
import com.catanddev.rtsp.codec.AudioFrameQueue
...
import com.catanddev.rtsp.codec.VideoDecodeThread.DecoderType
import com.catanddev.rtsp.codec.VideoDecodeThread.VideoDecoderListener
import com.catanddev.rtsp.codec.VideoDecoderSurfaceThread
import com.catanddev.rtsp.codec.VideoFrameQueue
...
class RtspProcessor(
    ...
    private var frameHandler: RtspController.RtspControllerCallbacks ? = null
) {
    ...
    // RTP server fields
    private var rtpServerHost: String = "0.0.0.0"
    private var rtpServerPort: Int = 8554
    ...
    // Статистика RTP
    val rtpStats = RtpStats()


    fun getRtpStats(): RtpStats.Stats {
    ...
    private val proxyClientListener = object: RtspClient.RtspClientListener {
        ...
        override fun onRtspConnecting() {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspConnecting()")
            uiHandler.post {
        ...
        override fun onRtspConnected(sdpInfo: SdpInfo) {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspConnected()")
            if (sdpInfo.videoTrack != null) {
                ...
                if (sps != null && pps != null) {
                    ...
                    if (RtspController.DEBUG)Log.d(TAG, "RTSP SPS and PPS NAL units missed in SDP")
                }
            }
            if (sdpInfo.audioTrack != null) {
                ...
                audioChannelCount = sdpInfo.audioTrack?.channels ?: 0
        ...
        override fun onRtspAudioSampleReceived(data: ByteArray, offset: Int, length: Int, timestamp: Long) {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspAudioSampleReceived(length=$length, timestamp=$timestamp)")
            if (length > 0) {
        ...
        override fun onRtspApplicationDataReceived(data: ByteArray, offset: Int, length: Int, timestamp: Long) {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspApplicationDataReceived(length=$length, timestamp=$timestamp)")
            dataListener?.onRtspDataApplicationDataReceived(data, offset, length, timestamp)
        }

        override fun onRtspDisconnecting() {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspDisconnecting()")
            uiHandler.post {
        ...
        override fun onRtspDisconnected() {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspDisconnected()")
            uiHandler.post {
        ...
        override fun onRtspFailedUnauthorized() {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspFailedUnauthorized()")
            uiHandler.post {
        ...
        override fun onRtspFailed(message: String?) {
            if (RtspController.DEBUG)Log.v(TAG, "onRtspFailed(message='$message')")
            uiHandler.post {
    ...
    inner class RtspThread: Thread() {
        ...
        fun stopAsync() {
            if (RtspController.DEBUG)Log.v(TAG, "stopAsync()")
        ...
        override fun run() {
            ...
            try {
                if (RtspController.DEBUG)Log.d(TAG, "Connecting to ${uri.host.toString()}:$port...")
                ...
                val rtspClient = RtspClient.Builder(socket, uri.toString(), rtspStopped, proxyClientListener)
                    ...
                    .withDebug((RtspController.DEBUG))
    ...
    private val videoDecoderListener = object: VideoDecoderListener {
        override fun onVideoDecoderStarted() {
            if (RtspController.DEBUG) Log.v(TAG, "onVideoDecoderStarted()")
        }

        override fun onVideoDecoderStopped() {
            if (RtspController.DEBUG)Log.v(TAG, "onVideoDecoderStopped()")
        }

        override fun onVideoDecoderFailed(message: String?) {
            if (RtspController.DEBUG)Log.e(TAG, "onVideoDecoderFailed(message='$message')")
        }

        override fun onVideoDecoderFormatChanged(width: Int, height: Int) {
            if (RtspController.DEBUG)Log.v(TAG, "onVideoDecoderFormatChanged(width=$width, height=$height)")
            rtpStats.setResolution(width, height)
            statusListener?.onRtspFrameSizeChanged(width, height)
        }

        override fun onVideoDecoderFirstFrameRendered() {
            if (RtspController.DEBUG)Log.v(TAG, "onVideoDecoderFirstFrameDecoded()")
            if (!firstFrameRendered) statusListener?.onRtspFirstFrameRendered()
            firstFrameRendered = true
            frameHandler?.onFrameAvailable()
        }
    }

    /**
     * Обновление Surface в активном видео декодере
     */
    fun updateDecoderSurface(newSurface: Surface) {
        videoDecodeThread?.let { decoder ->
            if (decoder is VideoDecoderSurfaceThread) {
                decoder.updateSurface(newSurface)
                Log.i(TAG, "Surface updated in video decoder")
            }
        }
    }

    /**
     * Перезапуск видео декодера с новым Surface
     */
    fun restartVideoDecoderWithNewSurface(newSurface: Surface) {
        if (videoDecodeThread != null && videoMimeType.isNotEmpty()) {
            Log.i(TAG, "Restarting video decoder with new Surface")

            // Останавливаем текущий декодер
            videoDecodeThread?.stopAsync()
            videoDecodeThread = null

            // Создаем новый декодер с новым Surface
            ...
            videoDecodeThread!!.apply {
                name = "RTSP video thread [${getUriName()}] (restarted)"
                start()
            }
        }
    }
    ...
    private fun onRtspClientStarted() {
        if (RtspController.DEBUG)Log.v(TAG, "onRtspClientStarted()")
    }

    private fun onRtspClientConnected() {
        if (RtspController.DEBUG)Log.v(TAG, "onRtspClientConnected()")
    ...
    private fun onRtspClientStopped() {
        if (RtspController.DEBUG)Log.v(TAG, "onRtspClientStopped()")
        stopDecoders()
        rtspThread = null
    }

    fun init(uri: Uri, username: String?, password: String?, userAgent: String? = null) {
        if (RtspController.DEBUG)Log.v(TAG, "init(uri='$uri', username='$username', password='$password', userAgent='$userAgent')")
        this.uri = uri
    ...
    fun setOnframeHandler(listener: RtspController.RtspControllerCallbacks?) {
    ...
    fun start(requestVideo: Boolean, requestAudio: Boolean, requestApplication: Boolean = false) {
        if (RtspController.DEBUG)Log.v(TAG, "start(requestVideo=$requestVideo, requestAudio=$requestAudio, requestApplication=$requestApplication)")
        if (rtspThread != null) rtspThread?.stopAsync()
        this.requestVideo = requestVideo
        this.requestAudio = requestAudio
        this.requestApplication = requestApplication

        // Проверяем, какой режим используем: RTSP или RTP
        if (::uri.isInitialized) {
            ...
            // RTP режим - запускаем декодеры сразу
            if (videoMimeType.isNotEmpty()) {
                firstFrameRendered = false
                Log.i(TAG, "Starting RTP video decoder with mime type \"$videoMimeType\"")
                videoDecodeThread = onVideoDecoderCreateRequested.invoke(
                    videoMimeType,
                    videoRotation,
                    videoFrameQueue,
                ...
                videoDecodeThread!!.apply {
                    name = "RTP video thread [${getUriName()}]"
                    start()
                }
            }
        }
    ...
    fun stop() {
        if (RtspController.DEBUG)Log.v(TAG, "stop()")
    ...
    fun isStarted(): Boolean {
        return rtspThread != null || videoDecodeThread != null
    }

    fun stopDecoders() {
        if (RtspController.DEBUG)Log.v(TAG, "stopDecoders()")
        videoDecodeThread?.stopAsync()
        videoDecodeThread = null
    ...
    private fun getNewLowLatencyFrameFromKeyFrame(frame: FrameQueue.VideoFrame): FrameQueue.VideoFrame {
        try {
            ...
            fun modifyVui() {
                spsSet.vuiParams = spsSet.vuiParams ?: VUIParameters()
    ...
    private fun getUriName(): String {
        ...
        // Для RTP режима
        return "RTP://$rtpServerHost:$rtpServerPort"
    }
