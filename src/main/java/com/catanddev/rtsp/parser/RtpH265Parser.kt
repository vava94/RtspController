...
class RtpH265Parser: RtpParser() {
    ...
    override fun processRtpPacketAndGetNalUnit(
        ...
        if (DEBUG) Log.v(TAG, "processRtpPacketAndGetNalUnit(offset=$offset, length=$length, marker=$marker)")

        // NAL Unit Header.type (RFC7798 Section 1.1.4).
        val nalType = ((data[offset].toInt() shr 1) and 0x3F).toByte()
...
//        Log.d(TAG, "\t\tNAL type: ${VideoCodecUtils.getH265NalUnitTypeString(nalType)}")
        ...
        if (nalType in 0..<RTP_PACKET_TYPE_AP) {
            ...
            nalUnit = processFragmentationUnitPacket(data, offset, length, marker)
        } else {
    ...
    private fun processFragmentationUnitPacket(data: ByteArray, offset: Int, length: Int, marker: Boolean): ByteArray? {
        if (DEBUG) Log.v(TAG, "processFragmentationUnitPacket(offset=$offset, length=$length, marker=$marker)")

        val fuHeader = data[offset + 2].toInt()
        ...
        if (isFirstFuPacket) {
            ...
            return addEndFragmentedPacketAndCombine(data, offset, length)
        } else {
    ...
    private fun addStartFragmentedPacket(data: ByteArray, offset: Int, length: Int) {
        ...
        fragmentedBuffer[0] = ByteArray(fragmentedBufferLength).apply {
            ...
            val tid = (data[offset + 1].toInt() and 0x7)
            val fuHeader = data[offset + 2].toInt()
        ...
        System.arraycopy(data, offset + 3, fragmentedBuffer[0]!!, 2, length - 3)
    }
    ...
    private fun addMiddleFragmentedPacket(data: ByteArray, offset: Int, length: Int) {
        ...
        if (fragmentedPackets >= fragmentedBuffer.size) {
            ...
            fragmentedBuffer[fragmentedPackets] = ByteArray(length - 3).apply {
                System.arraycopy(data, offset + 3, this, 0, length - 3)
            }
        }
    }
    ...
    private fun addEndFragmentedPacketAndCombine(data: ByteArray, offset: Int, length: Int): ByteArray? {
        ...
        if (fragmentedBuffer[0] == null) {
            ...
            // Write end packet
            System.arraycopy(data, offset + 3, nalUnit, tmpLen, length - 3)
            clearFragmentedBuffer()
